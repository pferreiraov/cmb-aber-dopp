...
def expected_error(cl,clfile=False,nlfile=False,mastermatrixfile=False,arcminbeam=0,lmin=3,lmax=2002,binsize=10,sufixname='test',threads=mp.cpu_count(),lmin_plot=1400,lmax_plot=2000,export_fg=False,export_sumbottom=False):
...
def expected_error2(cl,clfile=False,nlfile=False,mastermatrixfile=False,arcminbeam=0,lmin=3,lmax=2002,binsize=10,sufixname='test',threads=mp.cpu_count(),lmin_plot=1400,lmax_plot=2000,export_fg=False,export_sumbottom=False,export_plot=True):
    ...
    if nlfile != False:
        ...
        if mastermatrixfile != False:
            cl=cl[:mastermatrizsize]
            cl2=np.dot(mastermatrix,cl)[0:lmax+2] ## ATTENTION! Depends on n_l inputed (I'm using a masked n_l)
            cl = cl[0:lmax+2]+nl[0:lmax+2]
        else:
            nl=nl[:lmax+2]
            cl2=cl[:lmax+2]
            cl = cl2+nl
    if nlfile == False:
        if mastermatrixfile != False:
            cl=cl[:mastermatrizsize]
            cl2=np.dot(mastermatrix,cl)  
    if arcminbeam != 0:   
        gaussianbeam=hp.gauss_beam(radians(arcminbeam/60),lmax=lmax)
        gaussianbeam2=gaussianbeam*gaussianbeam
        cl2=cl/gaussianbeam2
        cl=cl2
    global cl_conv
    cl_conv = cl
